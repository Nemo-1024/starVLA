...
def lam_collate(batch: Sequence[Dict], max_state_dim: int = 32) -> Dict[str, Any]:
    ...
    - Each sample: {"frames": Tensor (T,H,W,C) uint8 NHWC, "proprio": Tensor (T,D) float32, "embodiment_id": int}.
    - Within a batch: all samples have the same (T,H,W,C), same T for proprio, and each D <= max_state_dim.
    ...
    for i, sample in enumerate(batch):
        ...
        if D > max_state_dim:
            raise ValueError(
                f"Proprio dim D={D} exceeds max_state_dim={max_state_dim} for sample index {i}."
            )
