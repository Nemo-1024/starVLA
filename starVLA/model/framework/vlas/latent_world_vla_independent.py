...
def _load_vlm_and_processor(
    ...
    vlm, processor = load_vlm_auto(vlm_model_id, cfg.hf_cache_dir, dtype=cfg.vlm_dtype)
    ...
    if placeholder_token_id < 0:
        raise ValueError(f"[LatentWorldPolicyBackend] invalid placeholder token id for `{placeholder_token}`")
...
def _apply_flow_only_grad_to_h_vlm(
    *,
    ...
    if flow_placeholder_mask is None:
        raise ValueError("[LatentWorldPolicyBackend] flow_only_mode=True requires flow_placeholder_mask.")
    if flow_placeholder_mask.dim() != 2:
        raise ValueError(
            f"[LatentWorldPolicyBackend] expected flow_placeholder_mask [B, L], got {tuple(flow_placeholder_mask.shape)}"
        )
    if flow_placeholder_mask.shape[0] != h_vlm.shape[0] or flow_placeholder_mask.shape[1] != h_vlm.shape[1]:
        raise ValueError(
            "[LatentWorldPolicyBackend] flow_placeholder_mask shape mismatch with h_vlm: "
...
class LatentWorldPolicyBackend(nn.Module):
    ...
    def __init__(self, model_cfg: LatentWorldPolicyConfig, *, vlm_model_id: str) -> None:
        ...
        # 2) Load LAM.
        self.lam = self._init_lam_from_cfg()
    ...
    def save_pretrained(self, save_directory: Union[str, Path], **kwargs):
        """
        注意：
        当前训练管线并未使用此函数保存checkpoint，
        而是直接 state_dict + torch.save() 保存
        """
    ...
    def _prepare_queries(self, *, device: torch.device, vlm_stage_dtype: torch.dtype) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if flow_query is None:
            raise ValueError("[LatentWorldPolicyBackend] flow_action_query is None; check ConditionalFlowMatchingHead.")
